    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 font-semibold">Manage your customer database and relationships.</p>
        </div>
        <button 
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {contacts.map((contact: any) => (
              <tr key={contact.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-all cursor-pointer group">
                ...
                <td className="px-6 py-4 text-sm font-medium text-slate-600 dark:text-slate-400">{contact.phone}</td>
                <td className="px-6 py-4">
                   <span className={cn(
